...
public class TimeLock implements Scrollable {

	long when;

	public TimeLock() {
		this.when = System.currentTimeMillis() + 300 * 1000;
	}
	...
	public String next() {
		...
		String msg = "§dTime until exit: §e" + part;
		if (done() || part.contains("no time")) {

			msg = "§aYou may now exit through The End Portal";
	...
	private String getUntrimmedTime(long i) {
		...
		if (seconds == 0 && minutes == 0)
			return (hours != 0 ? (hours == 1 ? hours + " hour " : hours + " hours ") : "no time at all");
		if (minutes == 0) {
			if (seconds == 1)
